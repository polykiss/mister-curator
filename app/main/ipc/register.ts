...
export function registerIpcHandlers(
  ...
  // active.
  //
  // feat/arcade-parity-2-metadata — pre-PR-62 the synthetic Arcade
  // row had no scrape target, so this handler early-returned for
  // `ARCADE_VIRTUAL_CORE_ID` to keep the engine's queue clean. PR
  // #62 wired up an actual arcade pass (deps.scrape dispatches to
  // `getArcadeMetadata` when coreId is the arcade sentinel), so the
  // guard is now actively wrong — it blocked the user from clicking
  // the Arcade sidebar row to pivot the queue. Guard removed; the
  // engine handles the sentinel coreId via the deps wiring.
